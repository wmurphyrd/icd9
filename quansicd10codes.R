quan_elix_icd10 <- function(data, code.column, id.column) {
  #Quan's SAS lists:
  comorbids<-list(
    ...
    htn=c(c('I10'),c('I11','I12','I13','I15')), #htn simple and complex merged
    paralysis=c('G041','G114','G801','G802','G81','G82','G830','G831','G832','G833','G834','G839'),
    neuro=c('G10','G11','G12','G13','G20','G21','G22','G254','G255','G312','G318','G319','G32','G35','G36','G37','G40','G41','G931','G934','R470','R56'),
    copd=c('I278','I279','J40','J41','J42','J43','J44','J45','J46','J47','J60','J61','J62','J63','J64','J65','J66','J67','J684','J701','J703'),
    dm_s=c('E100','E101','E109','E110','E111','E119','E120','E121','E129','E130','E131','E139','E140','E141','E149'),
    dm_cx=c('E102','E103','E104','E105','E106','E107','E108','E112','E113','E114','E115','E116','E117','E118','E122','E123','E124','E125','E126','E127','E128','E132','E133','E134','E135','E136','E137','E138','E142','E143','E144','E145','E146','E147','E148'),
    thyroid=c('E00','E01','E02','E03','E890'),
    renal=c('I120','I131','N18','N19','N250','Z490','Z491','Z492','Z940','Z992'),
    liver=c('B18','I85','I864','I982','K70','K711','K713','K714','K715','K717','K72','K73','K74','K760','K762','K763','K764','K765','K766','K767','K768','K769','Z944'),
    ...
    tumor=c('C00','C01','C02','C03','C04','C05','C06','C07','C08','C09','C10','C11','C12','C13','C14','C15','C16','C17','C18','C19','C20','C21','C22','C23','C24','C25','C26','C30','C31','C32','C33','C34','C37','C38','C39','C40','C41','C43','C45','C46','C47','C48','C49','C50','C51','C52','C53','C54','C55','C56','C57','C58','C60','C61','C62','C63','C64','C65','C66','C67','C68','C69','C70','C71','C72','C73','C74','C75','C76','C97'),
    arthritis=c('L940','L941','L943','M05','M06','M08','M120','M123','M30','M310','M311','M312','M313','M32','M33','M34','M35','M45','M461','M468','M469'),
  ...
  #match up the names with package icd9's labels for Elixhauser comorbidities
  #the order is the same because we both followed Quan's published order
  names(comorbids) <- icd9ComorbidQuanElix(data.frame(visitId="a",icd9="250.0"), 
                                           isShort=FALSE,return.df=TRUE) %>% select(-1) %>% names

  icd10_comorbidity(data,comorbids, code.column, id.column)
  
}

quan_charlson_icd10 <- function (data, code.column, id.column) {
  #Quan's SAS lists
  comorbids <- list (
    mi=c('I21','I22','I252'),
  ...
  #match up the names with package icd9's labels for Charlson comorbidities
  #the order is the same because we both followed Quan's published order
  names(comorbids) <- icd9ComorbidQuanDeyo(data.frame(visitId="a",icd9="250.0"), 
                                           isShort=FALSE,return.df=TRUE) %>% select(-1) %>% names
  icd10_comorbidity(data,comorbids, code.column, id.column)
}

icd10_comorbidity <- function(data, comorbids, code.column, id.column) {
  
  grep_any <- function(indexcodes, observedcodes, ...) {
    any(sapply(X=indexcodes,FUN=grepl,x=observedcodes,...))
  }
  get_comorbids <- function(subset,code.var) {

    sapply(X=comorbids,FUN=grep_any,observedcodes=sub("\\.", "", subset[[code.var]]))
  }
  ddply(data, .(Encounter.No.),get_comorbids,code.var=code.column)
}