...
test_that("expand icd9 range definition", {
  ...
  expect_equal(icd9ExpandRangeShort("V1009", "V101"), c("V1009", "V101", "V1010", "V1011",
...
test_that("icd9ChildrenDecimal", {
  ...
  expect_error(icd9ChildrenDecimal("1234", invalidAction = "stop")) # too long major
  expect_error(icd9ChildrenDecimal("V234", invalidAction = "stop")) # too long V major
  expect_error(icd9ChildrenDecimal("v101.1", invalidAction = "stop")) # too long major
  expect_error(icd9ChildrenDecimal("e123.45", invalidAction = "stop")) # wrong in three ways
  expect_error(icd9ChildrenDecimal("JACK", invalidAction = "stop")) # not number or V format
  expect_error(icd9ChildrenDecimal("123.456", invalidAction = "stop")) # too long minor
  expect_error(icd9ChildrenDecimal("9123.456", invalidAction = "stop")) # too long major and minor
  # if no error, at least give NA when possible for bad input, although we're
  # officially not checking input
  #   expect_equal(icd9ChildrenDecimal("1234", validate = FALSE), NA_character_) # too long major
  #   expect_equal(icd9ChildrenDecimal("V234", validate = FALSE), NA_character_) # too long V major
  #   expect_equal(icd9ChildrenDecimal("v101.1", validate = FALSE), NA_character_) # too long major
  #   expect_equal(icd9ChildrenDecimal("e123.45", validate = FALSE), NA_character_) # wrong in three ways
  #   expect_equal(icd9ChildrenDecimal("JACK", validate = FALSE), NA_character_) # not number or V format
